...
fn closest_freq(before: &Vec<TVO>, after: &Vec<(f32, f32)>) -> MatchResult {
    ...
    line_up(before, best.0, after, best.1)
}
...
impl Matcher {
    fn mtch(&self, before: &Vec<TVO>, after: &Vec<(f32, f32)>) -> MatchResult {
        match &self {

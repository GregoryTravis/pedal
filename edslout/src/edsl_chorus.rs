...
pub struct EdslChorus {
    unitConst_4: Const,
    unitLinearVibrato_3: LinearVibrato,
    unitConst_7: Const,
    ...
    signal9: Signal<f32>,
            ...
            impl EdslChorus {
                pub fn new() -> EdslChorus {
                    EdslChorus {
                            unitConst_4: Const::new(2.7f32),
    unitLinearVibrato_3: LinearVibrato::new(20usize, -24isize),
...
let port3_1: Window<f32> = Window::new(&self.signal5, Range(-49, 0));
self.unitLinearVibrato_3.go(playhead, &port3_0, &port3_1, &mut self.signal3);

self.unitConst_7.go(playhead, &mut self.signal7);

let port6_0: Window<f32> = Window::new(&self.signal7, Range(0, 0));
let port6_1: Window<f32> = Window::new(&self.signal5, Range(-53, 0));
...
let port2_1: Window<f32> = Window::new(&self.signal6, Range(0, 0));
...
let port8_1: Window<f32> = Window::new(&self.signal5, Range(-45, 0));
...
let port1_1: Window<f32> = Window::new(&self.signal8, Range(0, 0));
